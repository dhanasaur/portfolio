...
const Background = () => {
    ...
    useEffect(() => {
        if (IS_TOUCH) return; // no video setup on mobile

        const v = videoRef.current;
        if (v) v.playbackRate = 0.4;

        // NOTE: We deliberately removed the scroll→playbackRate handler.
        // Changing playbackRate on every scroll event causes compositor jank
        // (the browser has to constantly re-decode frames at variable rates).
        // A fixed slow playback looks almost identical and costs nothing.
        ...
        return () => clearInterval(checkPlayState);
    }, []);
    ...
    return (
        <div className="fixed inset-0 z-0 w-full h-full overflow-hidden pointer-events-none bg-black">
            <video
                ...
                loop
                playsInline
                // Hint to browser this video is background — deprioritise decode quality
                preload="auto"
